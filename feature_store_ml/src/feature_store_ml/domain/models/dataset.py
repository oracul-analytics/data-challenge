...
class FeatureDataset:
    ...
    def sample(self, n: int | None = None, frac: float | None = None, random_state: int | None = None) -> FeatureDataset:
        sampled_features = self.features.sample(n=n, frac=frac, random_state=random_state)
        sampled_labels = self.labels.loc[sampled_features.index] if self.labels is not None else None
    ...
    def add_labels(self, labels: pd.Series) -> FeatureDataset:
        if len(labels) != len(self.features):
            raise ValueError(f"Labels length {len(labels)} doesn't match features length {len(self.features)}")
