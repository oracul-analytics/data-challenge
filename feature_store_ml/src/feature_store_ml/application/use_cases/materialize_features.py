...
class MaterializeFeatures:
    ...
    def _get_feature_names(self) -> list[str]:
        ...
        for key in possible_keys:
            if key in self._metadata and self._metadata[key]:
                logger.info(
                    f"Using feature names from metadata['{key}']: {self._metadata[key]}"
                )
    ...
    def _predict_artifacts(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Using {len(feature_cols)} features for prediction: {feature_cols}"
        )
        ...
        logger.info(
            f"Prediction stats - Class 0: {(predictions == 0).sum()}, "
            f"Class 1: {(predictions == 1).sum()}"
        )
        logger.info(
            f"Score range: [{prediction_scores.min():.4f}, {prediction_scores.max():.4f}]"
        )
    ...
    def execute(
        ...
        logger.info(
            f"Detected {artifact_count} artifacts, keeping {len(clean_features)} clean records"
        )

        if clean_features.empty:
            logger.warning(
                "All records were classified as artifacts - nothing to write"
            )
            return
